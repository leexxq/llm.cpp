...
__global__ void LayerNormForwardCuda(float * outputs_d,float * means , float* rstds , float const * inputs_d,float const * gamma , float const * beta, int const length,int const stride){
    ...
    for(int offest = warpSize/2 ; offest > 0 ; offest/= 2){
        mean += __shfl_down_sync(mask,mean,offest);
        rstd +=__shfl_down_sync(mask,rstd,offest);
    }
...
void LayerNorm1D(float * outputs_d,float * means , float* rstds , float const * inputs_d,float const * gamma,float const * beta,int const length,int const stride){
    ...
    LayerNormForwardCuda<<<blocks,threads>>>(outputs_d,means,rstds,inputs_d,gamma,beta,length,stride); 
    CUDA_CHECK_LAST();
}

using DAlloc = cutlass::device_memory::allocation<float>;

void gpt2cuda::BatchLayerNormForward(float * outputs , float * means, float * rstds,float const * inputs,float const * gamma,float const * beta,int B,int T,int C){
    ...
    LayerNorm1D<>(outputs_d.get(),means_d.get(),rstds_d.get(),inputs_d.get(),gamma_d.get(),beta_d.get(),B*T*C,C);
...
__global__ void ComputeHatInputs(float*inputs,float const*means,float const* rstds,int B,int T,int C){
...
template<int K>
__launch_bounds__(256) __global__ void ComputeDerivateInputs(float* d_inputs,float const * d_outputs, float const* inputs,float const* gamma,float const * rstds,int B,int T,int C){
...
void DerivativeLayerNorm1D(float * d_inputs , float * d_gamma, float * d_beta, float const * d_outputs, float * inputs, float const * gamma,float const * means, float const * rstds, int B, int T, int C){

    assert(C!=0);
    //convert inputs to  \hat{x}_{ij}
    ComputeHatInputs<<<(B*T*C + threads - 1)/ threads,threads>>>(inputs,means,rstds, B,T,C);
    CUDA_CHECK_LAST();
    ...
    //compute d_inputs
    ComputeDerivateInputs<K><<<(B*T + K - 1) / K,threads,sizeof(float) * C>>>(d_inputs, d_outputs , inputs, gamma,rstds, B,T,C);
    CUDA_CHECK_LAST();

}


void gpt2cuda::BatchLayerNormBackward(float * d_inputs , float * d_gamma, float * d_beta, float const * d_outputs, float const * inputs, float const * gamma, float const * means, float const * rstds, int B, int T, int C){
    //input
    ...
    // inputs modified
    DerivativeLayerNorm1D<>(d_inputs_d.get(), d_gamma_d.get(), d_beta_d.get(),  d_outputs_d.get(),  inputs_d.get(),  gamma_d.get(),   means_d.get(),  rstds_d.get(),  B,  T,  C);
