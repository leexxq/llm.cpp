...
#define CUDA_CHECK(error) do{if(error != cudaSuccess){std::cout << cudaGetErrorString(error) << std::endl; throw cutlass::cuda_exception(cudaGetErrorString(error),error);}}while(false)