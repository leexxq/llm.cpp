...
void Layer::Init(size_t B, size_t T, size_t C, size_t V, size_t NH) {
	...
	dl_residual2_ = makeZero(B, T, C);
	dl_fcproj_ = makeZero(B, T, C);
	dl_fch_gelu_ = makeZero(B, T, 4 * C);
	dl_fch_ = makeZero(B, T, 4 * C);
	dl_ln2_ = makeZero(B, T, C);
	dresidual_ = makeZero(B, T, C);
	dl_attproj_ = makeZero(B, T, C);
	dl_residual_ = makeZero(B, T, C);
	dl_atty_ = makeZero(B, T, C);
	dl_qkv_ = makeZero(B, T, 3 * C);
	dl_ln1_ = makeZero(B, T, C);
}
...
VecBTC Layer::Backward(const VecBTC &d_outputs, const VecBTC &residual) {
	auto dresidual2_wrap = residual2.Backward(d_outputs);
	dl_residual2_ += dresidual2_wrap.first;
	dl_fcproj_ += dresidual2_wrap.second;

	assert(dl_residual2_.size() > 0);
	DEBUG_PRINTLN("residual2_ backward sharp: ({} , {} , {})", dl_residual2_.size(), dl_residual2_[0].rows(), dl_residual2_[0].cols());
	DEBUG_PRINTLN("dl_residual2_[0]: \n{}", fmt::streamed(dl_residual2_[0].block<2, 2>(0,0)));
	DEBUG_PRINTLN("dl_fcproj[0]: \n{}", fmt::streamed(dl_fcproj_[0].block<2, 2>(0,0)));

	dl_fch_gelu_ += fcproj.Backward(dl_fcproj_, l_fch_gelu_);
	assert(dl_fch_gelu_.size() > 0);
	DEBUG_PRINTLN("fch_gelu_ backward sharp: ({} , {} , {})", dl_fch_gelu_.size(), dl_fch_gelu_[0].rows(), dl_fch_gelu_[0].cols());
	DEBUG_PRINTLN("dl_fch_gelu[0]: \n{}", fmt::streamed(dl_fch_gelu_[0].block<2, 2>(0,0)));

	dl_fch_ += fch_gelu.Bacward(dl_fch_gelu_, l_fch_, l_fch_gelu_);
	assert(dl_fch_.size() > 0);
	DEBUG_PRINTLN("fch_gelu_ backward sharp: ({} , {} , {})", dl_fch_.size(), dl_fch_[0].rows(), dl_fch_[0].cols());
	DEBUG_PRINTLN("dl_fch_[0]: \n{}", fmt::streamed(dl_fch_[0].block<2, 2>(0,0)));

	dl_ln2_ += fch.Backward(dl_fch_, l_ln2_);
	assert(dl_ln2_.size() > 0);
	DEBUG_PRINTLN("fch_ backward sharp: ({} , {} , {})", dl_ln2_.size(), dl_ln2_[0].rows(), dl_ln2_[0].cols());
	DEBUG_PRINTLN("dl_ln2_[0]: \n{}", fmt::streamed(dl_ln2_[0].block<2, 2>(0,0)));

	dl_residual2_ += layernorm2.Backward(dl_ln2_, l_residual2_);
	assert(dl_residual2_.size() > 0);
	DEBUG_PRINTLN("layernorm2_ backward sharp: ({} , {} , {})", dl_residual2_.size(), dl_residual2_[0].rows(), dl_residual2_[0].cols());
	DEBUG_PRINTLN("dl_residual2_[0]: \n{}", fmt::streamed(dl_residual2_[0].block<2, 2>(0,0)));

	auto dresidual1_wrap = residual1.Backward(dl_residual2_);
	dl_attproj_ += dresidual1_wrap.first;
	dl_residual_ += dresidual1_wrap.second;
	assert(dl_attproj_.size() > 0);
	DEBUG_PRINTLN("residual1_ backward sharp: ({} , {} , {})", dl_attproj_.size(), dl_attproj_[0].rows(), dl_attproj_[0].cols());
	assert(dl_residual2_.size() > 0);
	DEBUG_PRINTLN("dl_attproj_[0]: \n{}", fmt::streamed(dl_attproj_[0].block<2, 2>(0,0)));
	DEBUG_PRINTLN("dl_residual_[0]: \n{}", fmt::streamed(dl_residual_[0].block<2, 2>(0,0)));

	dl_atty_ += att_proj.Backward(dl_attproj_, l_atty_);
	assert(dl_atty_.size() > 0);
	DEBUG_PRINTLN("att_proj_ backward sharp: ({} , {} , {})", dl_atty_.size(), dl_atty_[0].rows(), dl_atty_[0].cols());
	DEBUG_PRINTLN("dl_atty_[0]: \n{}", fmt::streamed(dl_atty_[0].block<2, 2>(0, 0)));

	dl_qkv_ += attention.Backward(dl_atty_, l_qkv_);
	assert(dl_qkv_.size() > 0);
	DEBUG_PRINTLN("attention_ backward sharp: ({} , {} , {})", dl_qkv_.size(), dl_qkv_[0].rows(), dl_qkv_[0].cols());
	DEBUG_PRINTLN("dl_qkv_[0]: \n{}", fmt::streamed(dl_qkv_[0].block<8, 8>(0,0)));

	dl_ln1_ += qkv.Backward(dl_qkv_, l_ln1_);
	assert(dl_ln1_.size() > 0);
	DEBUG_PRINTLN("qkv_ backward sharp: ({} , {} , {})", dl_ln1_.size(), l_ln1_[0].rows(), l_ln1_[0].cols());
	DEBUG_PRINTLN("dl_ln1_[0]: \n{}", fmt::streamed(dl_ln1_[0].block<2, 2>(0,0)));

	VecBTC d_inputs = layernorm1.Backward(dl_ln1_, residual);
